...
public class ImageProcessingService {
    ...
    private Mat removal_func(Mat resized, Mat mask, Mat bgModel, Mat fgModel, Mat source) {
        ...
        // Step 5: Find edges using Sobel (adjust kernel size for less aggressive edge detection)
        Mat edges = new Mat();
        ...
        for (int i = 0; i < rgba.rows(); i++) {
            for (int j = 0; j < rgba.cols(); j++) {
                ...
                // Check if pixel is near the edge (transparency effect based on distance to edge)
